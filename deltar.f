...
c-----------------------------------------------------------------------
     ...
     $     nps=8,nxps=1
      INTEGER :: ntmax=1
      REAL(r8) :: fmin=1,fmax=1,rtol=1e-8,atol=1e-6,tmax

      REAL(r8), PRIVATE :: ee,ff,hh,mm,gg,kk,di,dr,tmin,pplus,p1
      REAL(r8), DIMENSION(neqc), PRIVATE :: al0
      COMPLEX(r8), PRIVATE :: q
      COMPLEX(r8), DIMENSION(6), PRIVATE :: dtr
      COMPLEX(r8), DIMENSION(neqc) :: pexp,bl1
...
c-----------------------------------------------------------------------
      ...
      CASE(4)
         tmax=SQRT(ABS(p1/lamda))
      CASE DEFAULT
         CALL program_stop("deltar_infinity: illegal ntmax_type")
      END SELECT
      tmax=tmax*fmax
c-----------------------------------------------------------------------
c     write output.
